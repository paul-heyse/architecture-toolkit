...
| Static typing | Pyrefly over src/tests/scripts, 0 errors; strict `src` coverage 100.00%; `pyarrow-stubs` qualified statically and at run time with two divergences pinned | D-032 / Pyrefly CORE-53..62 | migrated; strict coverage enforced at 100 in CI |
| DATA Arrow/Delta | twelve declared schemas, bidirectional mappings, chunk-independent digests, the metadata policy, versioned Delta snapshots with commit provenance and per-table idempotency, manifest-driven retention and explicit migrations (W3, W4) | DATA-01..60 | landed at W5 and W6 |
| Snapshot provider | four rungs — materialized, PyArrow Dataset, Arrow stream and a native FFI provider that refuses under this lock — all run over a published release's twelve tables; materialized remains the default (W4) | materialized fallback + qualified Dataset/stream candidates | proven; the default is unchanged by design |
...
| Release publication | immutable manifest with a linked parent chain, one-writer lock, expected-parent check, staging with version reuse, read-back verification, an atomic current pointer, fault injection at all eight stages, and `resume`/`discard` for the crash window (W4) | coherent manifest/lock/staging/failure protocol | proven for M2 |
| Semantic diff/migration | versioned canonical hash and record-level delta (W2); explicit versioned storage migrations with historical replay, and Delta CDF as a cross-check rather than the narrative (W4) | typed change classification, scenarios, historical migrations | landed at W6 |
| Jinja text generation | one deterministic environment with the two nondeterministic builtins removed, transitive template-bundle digests, a template/DTO contract check that also catches unknown filters, and one generated Markdown artifact with full provenance (W7a) | CORE-32..38 | proven for the text path; no notation is generated |
| lxml standards XML | central namespace registry and `ElementMaker`, one secure parser baseline, a checksummed resolver proven against both the BPMN and ArchiMate schema sets offline, schema findings as `Diagnostic`s, and C14N 2.0 digests stable across pretty-printing (W7a) | CORE-39..44 | proven; no standards document is generated yet |
| Projection model | `ViewDefinition` canonical on `Model` with four membership rules, and the five provenance records pinned by digest rather than tabled (W7a) | PROJ-01..07 | landed at W7a |
