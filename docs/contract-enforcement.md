...
| `jinja-environment-is-central` | CORE-33 — one constructed `Environment`, so every setting that affects generated bytes is explicit in one place and the two nondeterministic builtins are removed once; the exclusion glob is proven by `tests/unit/test_layering.py` |
...
| CORE-49 | A requirement's evidence is a test that *ran* | `scripts/check_evidence.py` reads the evidence document the pytest plugin writes and refuses a run in which a requirement-marked test skipped. A skip is green, so a requirement can go from evidenced to unevidenced without a red build — which is what happened to CORE-40, CORE-41 and CORE-42 for two commits when CI ran pytest before it vendored the schemas |
