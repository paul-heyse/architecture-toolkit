...
if TYPE_CHECKING:
    ...
    from architecture_toolkit.projections.generators import BuiltProjection
...
class ProjectionGenerator(Protocol):
    ...
    **It returns both the bytes and the record**, and writes neither. A generator returning only a
    string would leave every caller to reconstruct the provenance — which model digest, which
    generator version, which mapping profile, which template bundle — and PROJ-05 exists so nobody
    has to. A generator that wrote the file itself would be one whose output depends on the
    filesystem, which is the determinism W7a spent a wave establishing. The caller decides where
    the bytes go.

    **It takes a `Model`, not a store.** Reading a release is the pipeline's job, and a generator
    that could reach a store could reach anything in it. This is what keeps every generator
    testable from a model alone.

    `view_id` may be `None`: ArchiMate Exchange XML is generated per *model*, not per view, and
    requiring one would force that generator to invent it. `projections/generators.py` holds the
    implementations and the registry `architecture build --notation` selects from.
    """
